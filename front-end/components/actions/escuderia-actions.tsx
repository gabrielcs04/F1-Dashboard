...
export function EscuderiaActions({ user }: EscuderiaActionsProps) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900">Ações - {user.name}</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Button onClick={() => setShowConsultaForm(true)} className="h-20 flex flex-col">
          <span className="font-semibold">Consultar por Forename</span>
          <span className="text-sm opacity-80">Buscar piloto por nome</span>
        </Button>
